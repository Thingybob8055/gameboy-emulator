...
typedef struct
{
    ...
    //other data...
    u32 bg_colors[4];
    u32 sp1_colors[4]; //essentialy same as obj_pallete[0] and [1]
    u32 sp2_colors[4]; //
...
//enum to track different LCD modes
//macros to access lcd control values according to pandocs
#define LCDC_BGW_ENABLE (BIT(lcd_get_context()->lcdc, 0)) //returns bit 0 of lcdc
...
//says what mode LCD is in. //grab first 2 bits and cast to LCD mode
