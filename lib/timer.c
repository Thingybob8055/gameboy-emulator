...
void timer_init() {
    ctx.div = 0xAC00; //default value
}

void timer_tick() {
    u16 prev_div = ctx.div;

    ctx.div++; //increment the div

    bool timer_update = false; //check if we need to update timer or not

    //switching and checking the bottom two bits
    switch(ctx.tac & (0b11)) {
        case 0b00:
